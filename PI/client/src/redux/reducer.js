  ...
  switch (type) {
    ...
    case SEARCH_COUNTRY:
      ...
      if (state.countries !== state.allCountries) {
        PaisesBuscados = paisesfilt.filter((country) =>
    ...
    case FILTER_COUNTRY:
      ...
      if (payload.continent !== "All") {
        countriesFiltrados = countriesFiltrados.filter(
          (el) => el.continent === payload.continent
        );
      }

      if (payload.activity !== "All") {
        countriesFiltrados = countriesFiltrados.filter((country) =>
          country.Activities.find(
            (activity) => activity.name === payload.activity
          )
        );
      }
